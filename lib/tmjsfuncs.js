...
function parseVisibleData(data) {
	...
	if (match >= 0) {
	    console.log("Found concurrency. Existing: " + match + " " + connections[match].TMpopupinfo + " New: " + roots[i]);
	    connections[match].TMpopupinfo += " " + routelistnames[i];
	...
	for (let j = 0; j < routeroots.length; j++) {
		...
		con.TMpopupinfo = routelistnames[j];
		break;
	    }
	}
	connectionListener({
