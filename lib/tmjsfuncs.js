...
function parseVisibleData(data) {
	...
	if (match >= 0) {
	    console.log("Found concurrency. Existing: " + match + " " + connections[match].info + " New: " + roots[i]);
	...
	con.end1 = w1name;
	con.end2 = w2name;
	// find the route for this segment's root
	for (let j = 0; j < routeroots.length; j++) {
	    if (routeroots[j] == roots[i]) {
		con.info = routelistnames[j];
		break;
	    }
	}
	connectionListener({
	    connIndex: connections.length - 1
	});
    }
}
