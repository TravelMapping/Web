...
function parseVisibleData(data) {
	...
	if (routes[root].systemcode != currentSystem) {
	    ...
	    tablerows += '<tr><td colspan="4" style="text-align: center; background-color: ' + bgcolor + '">' + routes[root].systemname + " (" + currentSystem + ')</td></tr>';
	}
	let link = "/hb/?r=" + root;
	tablerows += '<tr onclick="window.open(\'' + link + '\')" onmouseover="mapviewRouteHoverRoot(\'' + root + '\')" onmouseout="mapviewRouteEndHoverRoot()"><td class="routeName">' + routes[root].listname + '</td><td class="clinched">TBD</td><td class="overall">' + routes[root].mileage + '</td><td class="percent">TBD</td></tr>';
    }
...
*/
	...
	if (match >= 0) {
	    if (!connections[match].TMrouteshere.includes(root)) {
		// are we at a "smaller" tier than previous?
		if (routes[root].tier < connections[match].TMminTier) {
		    // our new min tier
		    connections[match].TMminTier = routes[root].tier;
		    // update color based on this route
		    connections[match].setStyle({
		    ...
		    connections[match].TMrouteshere.unshift(root);
		    ...
		    // connection
		    let points = connections[match].getLatLngs();
			...
			if (oldw1loc.equals(w1latlng)) {
			    foundw1 = true;
			    connections[match].TMend1 = w1name[checkNext];
			}
			if (oldw2loc.equals(w1latlng)) {
			    foundw2 = true;
			    connections[match].TMend2 = w1name[checkNext];
			}
			let w2latlng = L.latLng([w2lat[checkNext],
						 w2lng[checkNext]]);
			if (oldw2loc.equals(w2latlng)) {
			    foundw2 = true;
			    connections[match].TMend2 = w2name[checkNext];
			}
			if (oldw1loc.equals(w2latlng)) {
			    foundw1 = true;
			    connections[match].TMend1 = w2name[checkNext];
			}
			checkNext++;
		    }
		}
		else {
		    connections[match].TMrouteshere.push(root);
		}
	    }
	...
	c.TMpopupinfo = routes[c.TMrouteshere[0]].listname;	
	c.TMpopupinfo += "<br />" + c.TMend1 + " <-> " + c.TMend2;
	// append the routes for this segment's additional roots
	for (let others = 1; others < c.TMrouteshere.length; others++) {
	    c.TMpopupinfo += "<br />" + routes[c.TMrouteshere[others]].listname;
	}
	connectionListener({
	    connIndex: i
	});
