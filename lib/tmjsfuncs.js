...
function loadmap() {
    ...
    var maptypelist = ['MQOpenMap', 'MQOpenSat', 'Mapnik', google.maps.MapTypeId.ROADMAP, google.maps.MapTypeId.SATELLITE, google.maps.MapTypeId.HYBRID, google.maps.MapTypeId.TERRAIN, 'Blank'];
...
function updateMap()
{
	...
	for (var i = 0; i < waypoints.length; i++) {
		...
		if (thisEdge.v1 == i) {
		    ...
		    connections[edgeNum] = new google.maps.Polyline({path: edgePoints, strokeColor: "#0000FF", strokeWeight: 10, strokeOpacity: 0.4, map: map});
		    edgeNum++;
		}
	    }
	}
