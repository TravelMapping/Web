...
// Global AJAX prefilter to automatically append dbname parameter to PHP requests
// This ensures all AJAX calls to PHP files that include tmphpfuncs.php get the dbname parameter
if (typeof $ !== 'undefined') {
    $.ajaxPrefilter(function(options, originalOptions, jqXHR) {
        // Only modify requests to PHP files that need dbname
        if (options.url && options.url.includes('.php')) {
            var dbname = null;
            
            // Try different sources for the database name
            if (typeof showrouteParams !== 'undefined' && showrouteParams.dbname) {
                dbname = showrouteParams.dbname;
            }
            // Check if there's a global tmdbname variable
            else if (typeof tmdbname !== 'undefined') {
                dbname = tmdbname;
            }
            // Check URL parameters of current page
            else {
                var urlParams = new URLSearchParams(window.location.search);
                if (urlParams.has('dbname')) {
                    dbname = urlParams.get('dbname');
                }
            }
            
            // If we found a dbname, append it to the request
            if (dbname) {
                var separator = options.url.includes('?') ? '&' : '?';
                options.url += separator + 'dbname=' + encodeURIComponent(dbname);
            }
        }
    });
}
...
var distanceUnits = getTMCookie("units");
...
colorCodes[5] = { name: "TMgreen", unclinched: "rgb(80,175,80)", clinched: "rgb(0,180,0)" };
...
colorCodes[10] = { name: "TMpurple", unclinched: "rgb(200,130,180)", clinched: "rgb(160,90,140)" };
colorCodes[11] = { name: "TMorange", unclinched: "rgb(255,180,50)", clinched: "rgb(255,165,0)" };
colorCodes[12] = { name: "TMblack", unclinched: "rgb(150,150,150)", clinched: "rgb(0,0,0)" };
...
function setTMCookie(name, value) {
	...
	+ d.toUTCString() + ";path=/;secure;samesite=Lax";
}
...
// return the current traveler in a span so it can be updated by
// setUserUnitsCookiesAndText
function travelerInSpan() {

    return '<span class="tmUserText">' + traveler + '</span>';
}

// return the distance units string in a span so it can be updated by
// setUserUnitsCookiesAndText
function distanceUnitsInSpan() {

    return '<span class="tmUnitsText">' + distanceUnits + '</span>';
}

// get the user and units from the standard dropdown menus and set the
// globals traveler and distanceUnits, store the cookies accordingly,
// and update any currently displayed text showing the traveler or units.
// also updates the browser's displayed URL with these changes.
function setUserUnitsCookiesAndText() {

    const usersel = document.getElementById("tmuserselect");
    const unitssel = document.getElementById("tmunitsselect");
    const url = new URL(window.location);
    if (usersel != null) {
        traveler = usersel.value;
        setTMCookie("lastuser", traveler);
        url.searchParams.set('u', traveler);
    }
    if (unitssel != null) {
    ...
    document.querySelectorAll('.TMuserText').forEach(el => {
         el.innerHTML = traveler;
    });
    document.querySelectorAll('.TMunitsText').forEach(el => {
         el.innerHTML = distanceUnits;
    });
}
