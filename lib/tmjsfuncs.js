...
function loadmap() {
    ...
    console.log("loadmap, mapStatus: " + mapStatus);
    // Leaflet map object
    map = L.map('map', {
	renderer: L.canvas()});
...
function parseVisibleData(data) {
    ...
    // segments['roots'] is the root of each segment
    // segments['routes'] is the list file name of each segment
    // segments['w1name'] name of one end waypoint
    // segments['w1lat'] latitude of one end waypoint
    // segments['w1lng'] longitude of one end waypoint
    // segments['w2name'] name of other end waypoint
    // segments['w2lat'] latitude of other end waypoint
    // segments['w2lng'] longitude of other end waypoint
    //
    ...
    let routes = segments['routes'];
    let w1name = segments['w1name'];
    let w1lat = segments['w1lat'];
    let w1lng = segments['w1lng'];
    let w2name = segments['w2name'];
    let w2lat = segments['w2lat'];
    let w2lng = segments['w2lng'];
    ...
    // loop through all segments and plot
...
function connectionInfo(arg){
    ...
    if (connections[i].hasOwnProperty("info")) {
	info += connections[i].info;
    }
