...
function parseVisibleData(data) {
    ...
    // responses['routesystems'] the systems (displayable) for routes  
    // responses['routelevels'] the levels (active or preview) for routes
    //
    // the elements here are ordered by csv order (first
    ...
    let routesystems = responses['routesystems'];
	...
	if (routesystems[i] != currentSystem) {
	    currentSystem = routesystems[i];
	    tablerows += '<tr><td colspan="4" style="text-align: center">' + currentSystem + '</td></tr>';
	}
