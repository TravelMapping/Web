...
// are we in mapview and loading data on scroll/zoom instead of
// bring provided by waypointsFromSQL?
...
function loadmap() {
    ...
    // College
    map.setView([42.719450, -73.752063], 14);
...
function updateMap(lat, lon, zoom)
{
    ...
    // set our map view..., but only if we had waypoints loaded by the
    // waypointsFromSQL function (which we would not if in a
    // scrollable mapview)
    if (!showAllInView) {
...
function zoomChange() {
    ...
    // so no need to update old connections
    if (showAllInView) {
	updateVisibleData();
    }
...
function parseVisibleData(data) {
    ...
    // responses['clinched'] is this segment clinched by the traveler
    ...
    let routelistnames = responses['routelistnames'];
    let routemileages = responses['routemileages'];
    let routecolors = responses['routecolors'];
    let routesystemnames = responses['routesystemnames'];
    let routesystemcodes = responses['routesystemcodes'];
    let routelevels = responses['routelevels'];
    let routetiers = responses['routetiers'];
    ...
    thead.innerHTML = '<tr class="float"><th class="sortable routeName">Route</th><th class="sortable clinched">Clinched ('+ distanceUnits + ')</th><th class="sortable overall">Overall (' + distanceUnits + ')</th><th class="sortable percent">%</th></tr>';
    let tbody = table.getElementsByTagName("tbody")[0];
    // dummy entry to handle styling problem
    let tablerows = '<tr><td colspan="4">&nbsp;</td></tr><tr><td colspan="4">&nbsp;</td></tr>';
    let currentSystem = "";
    for (let i = 0; i < routeroots.length; i++) {
	// system "header" row needed?
	if (routesystemcodes[i] != currentSystem) {
	    currentSystem = routesystemcodes[i];
	    let bgcolor = "#CCFFCC";  // active
	    if (routelevels[i] == "preview") {
		bgcolor = "#FFFFCC";
	    }
	    tablerows += '<tr><td colspan="4" style="text-align: center; background-color: ' + bgcolor + '">' + routesystemnames[i] + " (" + currentSystem + ')</td></tr>';
	}
	let link = "/hb/?r=" + routeroots[i];
	tablerows += '<tr onclick="window.open(\'' + link + '\')" onmouseover="mapviewRouteHoverRoot(\'' + routeroots[i] + '\')" onmouseout="mapviewRouteEndHoverRoot()"><td class="routeName">' + routelistnames[i] + '</td><td class="clinched">TBD</td><td class="overall">' + routemileages[i] + '</td><td class="percent">TBD</td></tr>';
    }
    ...
    // loop through all segments and build the appropriate markers
    // and connections
    ...
    let currentRoot = "";
    // and index into the routes arrays for the current route
    let currentRouteIndex = -1;
    let colors;
    for (let i = 0; i < routeroots.length; i++) {
	if (roots[0] == routeroots[i]) {
	    currentRouteIndex = i;
	    colors = lookupColors(routecolors[i], routetiers[i],
				  routesystemcodes[i]);
	    break;
	}
    }
...
*/
	...
	if (match >= 0) {
	    if (!connections[match].TMrouteshere.includes(roots[i])) {
		connections[match].TMrouteshere.push(roots[i]);
		// are we at a "smaller" tier than previous?
		let routeIndex = routeroots.indexOf(roots[i]);
		if (routetiers[routeIndex] < connections[match].TMminTier) {
		    connections[match].TMminTier = routetiers[routeIndex];
		    // color based on this one
		    let tempcolors = lookupColors(routecolors[routeIndex],
						  routetiers[routeIndex],
						  routesystemcodes[routeIndex]);
		    connections[match].setStyle({color: tempcolors[clinched[match]]});
		}
	    }
	...
	if (roots[i] != currentRoot) {
	    ...
	    currentRoot = roots[i];
	    for (let r = 0; r < routeroots.length; r++) {
		if (roots[i] == routeroots[r]) {
		    currentRouteIndex = r;
		    colors = lookupColors(routecolors[r], routetiers[r],
					  routesystemcodes[r]);
		    break;
		}
	    }
	...
	con.TMend1 = w1name[i];
	con.TMend2 = w2name[i];
	con.TMrouteshere = [ roots[i] ];
	con.TMminTier = routetiers[currentRouteIndex];
    }
    // build TMpopupinfo for each connection and add connectionListener
    for (let i = 0; i < connections.length; i++) {
	// find route for the first connected route here
	for (let j = 0; j < routeroots.length; j++) {
	    if (routeroots[j] == connections[i].TMrouteshere[0]) {
		connections[i].TMpopupinfo = routelistnames[j];
		break;
	    }
	}
	connections[i].TMpopupinfo += "<br />" + connections[i].TMend1 + " <-> " + connections[i].TMend2;
	// find the routes for this segment's additional roots
	for (let others = 1; others < connections[i].TMrouteshere.length; others++) {
	    for (let j = 0; j < routeroots.length; j++) {
		if (routeroots[j] == connections[i].TMrouteshere[others]) {
		    connections[i].TMpopupinfo += "<br />" + routelistnames[j];
		    break;
		}
	    }
	}
...
function intersectingConcurrentRoutes(i, atCenter) {
	...
	for (var j = 0; j < waypoints[i].intersecting.length; j++) {
	    ...
	    intersections+="<a href=\"/hb/?r=" + r.root + "&lat=" + lat + "&lon=" + lon + "&zoom=" + map.getZoom() + "\">" + r.region + " " + r.route;
	    if (r.banner != "") {
...
// for scrollable mapview, highlighting route segments when route is
// hovered over in the table of routes (here, we have no routeInfo or
// waypoints array
