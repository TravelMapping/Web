...
function updateMap(lat, lon, zoom)
{
	...
	// using this dialog box, using code from
...
Find place: <input id="placeinput" name="placeinput" type="text" size="20" maxlength="100" />&nbsp;<input id="findbutton" name="findbutton" type="button" value="Lookup" onclick="nominatimLookup('placeinput');" /><br />
(Geocoding by <a target="_blank" href="https://nominatim.openstreetmap.org/">OSM Nominatim</a>)
</p>
`,
	    prompt: {
		buttonAction: "OK",
		...
		action: function() {
		    ...
		    // from this one
		    updateMap(setlat, setlon, setzoom);
		},
		callback: function() {
...
function parseVisibleData(data) {
	...
	if (routes[root].systemcode != currentSystem) {
	    ...
	    tablerows += '<tr onmouseover="mapviewRouteHoverSystem(\'' + currentSystem + '\')" onmouseout="mapviewRouteEndHoverRoot()"><td colspan="4" style="text-align: center; background-color: ' + bgcolor + '">' + routes[root].systemname + " (" + currentSystem + ')</td></tr>';
	}
	...
	let link = "/hb/?r=" + root;
	tablerows += '<tr onclick="window.open(\'' + link + '\')" onmouseover="mapviewRouteHoverRoot(\'' + root + '\')" onmouseout="mapviewRouteEndHoverRoot()"><td class="routeName">' + routes[root].listname + '</td><td class="clinched">' + convertToCurrentUnits(routes[root].clinchedmileage).toFixed(2) + '</td><td class="overall">' + convertToCurrentUnits(routes[root].mileage).toFixed(2) + '</td><td class="percent">' + percent.toFixed(1) + '%</td></tr>';
    }
	...
	if (match >= 0) {
		...
		if (routes[root].tier < matched.TMminTier) {
			...
			color: lookupColors(routes[root].color,
					    routes[root].tier,
					    routes[root].systemcode)[clinched[match]]
		    });
...
// for scrollable mapview, highlighting route segments for all routes
