...
function updateMap()
{
	...
	if (newRouteIndices.length > 0) {
		...
		for (var i=start; i<end; i++) {
		    ...
		    // look for any previous polyline at the exact same location
		    // and make it transparent if found - only need to find most
		    // recently added as its addition would have taken care of
		    // any even earlier match

		    // straighforward implementation with linear search takes 56 seconds for
		    // http://tmtest2.teresco.org/user/mapview.php?units=miles&u=mapcat&rg=KY
		    // 64 seconds when using segmentLengths as property as a string
		    // to decide whether to do a search
		    // 70 seconds using segment length floating point property, ugh
		    if (segmentLengths.hasOwnProperty(segmentLength)) {
			console.log("have property " + segmentLength);
		    for (var prev = nextSegment - 1; prev >= 0; prev--) {
			let latLngs = connections[prev].getLatLngs();
			...
			if ((waypoints[i].lat == latLngs[0].lat &&
			    ...
			    connections[prev].setStyle({opacity: 0});
			    break;
			}
		    }
		    }
		    else {
			console.log("setting property " + segmentLength);
			segmentLengths[segmentLength] = true;
		    }
                    nextSegment++;
		}
	    }
	    ...
	    // weight in response
	    console.log("property count: " + Object.keys(segmentLengths).length);
	    map.on('zoomend', zoomChange);
	    zoomChange();
	}
	else {
