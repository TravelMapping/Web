...
function set_units_cookie() {

    // we'll have the cookie remain for a year
    var d = new Date();
    d.setTime(d.getTime() + 365*24*60*60*1000);
    document.cookie = "units=" + distanceUnits + ";expires="
	+ d.toUTCString() + ";path=/";
}

function set_traveler_cookie() {

    // we'll have the cookie remain for a year
    var d = new Date();
    d.setTime(d.getTime() + 365*24*60*60*1000);
    document.cookie = "lastuser=" + traveler + ";expires="
	+ d.toUTCString() + ";path=/";
}
...
var updateVisibleDataDebounced = debounce(updateVisibleData, 1000);
...
function parseVisibleData(data) {
    ...
    thead.innerHTML = '<tr><th rowspan="2">Route</th><th colspan="2">Clinched ('+ distanceUnits + ')</th></tr><tr><th>Visible Segments</th><th>Route Overall</th></tr>';
	...
	let link = "/hb/?r=" + root;
	tablerows += '<tr onclick="window.open(\'' + link + '\')" onmouseover="mapviewRouteHoverRoot(\'' + root + '\')" onmouseout="mapviewRouteEndHoverRoot()"><td>' + routes[root].listname + '</td><td>' + convertToCurrentUnits(routes[root].visibleclinched).toFixed(2) + ' of ' + convertToCurrentUnits(routes[root].visiblemileage).toFixed(2) + ' (' + vispercent.toFixed(1) + '%)</td><td>' + convertToCurrentUnits(routes[root].clinchedmileage).toFixed(2) + ' of ' + convertToCurrentUnits(routes[root].mileage).toFixed(2) + ' (' + percent.toFixed(1) + '%)</td></tr>';
    }
