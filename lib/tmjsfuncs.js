...
// on a map resize, we'll need to reload if showing all in view
function mapResized() {

    if (showAllInView) updateVisibleData();
}
    
// on a map move, we'll need to reload if showing all in view
function mapMoved() {
...
function parseVisibleData(data) {
    ...
    let tablerows = '<tr><td class="routeName">&nbsp;</td><td class="link systemName">&nbsp;</td><td class="clinched" style="width: 70px;">&nbsp;</td><td class="overall" style="width: 65px;">&nbsp;</td><td class="percent">&nbsp;</td></tr>';
    for (let i = 0; i < routeroots.length; i++) {
	let link = "/hb/?r=" + routeroots[i];
	tablerows += '<tr onclick="window.open(\'' + link + '\')" onmouseover="mapviewRouteHover(' + routeroots[i] + ')" onmouseout="mapviewRouteEndHover()"><td class="routeName">' + routelistnames[i] + '</td><td class="link systemName">' + routetiers[i] + '</td><td class="clinched">TBD</td><td class="overall">' + routemileages[i] + '</td><td class="percent">TBD</td></tr>';
    }
