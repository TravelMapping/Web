...
function updateMap()
{
	...
	if (routeInfo.length > 0) {
		...
		for (var i=start; i<end; i++) {
		    ...
		    // TODO: find concurrent routes and push also here
		    ...
		    for (let point = segStart + 1; point < segEnd - 1; point++) {
    ...
    // don't think this should be needed, but an attempt to get
...
function connectionInfo(arg){
	...
	info += '<span style="font-size:24pt; color:black">' +
	    ...
	    '<br /><b>Distance: </b>' +
	    edgeLengthInMiles(graphEdges[i]).toFixed(4) + ' mi';
    }
	...
	info += '<span style="font-size:18pt; color:black">' +
	    '<a href="/hb?r=' + routeInfo[r].root + '">' + routeInfo[r].label +
	    '</a></span><br />' +
	    arg.endpoints + "<br /><b>Length: </b>" + arg.length.toFixed(4);
	if (arg.routes.length > 1) {
	    info += "<br /><b>Concurrent with:</b> ";
	}
	for (let j = 1; j < arg.routes.length; j++) {
	    info += '<a href="/hb?r=' + routeInfo[arg.routes[j]].root + '">' +
	...
	for (let point = start + 1; point < end - 1; point++) {
	    length += distanceInMiles(waypoints[point].lat,
				      waypoints[point].lon,
				      waypoints[point+1].lat,
				      waypoints[point+1].lon);
