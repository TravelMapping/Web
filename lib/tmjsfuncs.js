...
function parseVisibleData(data) {
    ...
    // responses['routes'] is the list file name of each segment
    ...
    // might also need to remove entries from the routes table

    console.log("Adding " + roots.length + " segments");
    console.log("Have " + routeroots.length + " unique routes");
	...
	if (match >= 0) {
	    console.log("Found concurrency. Existing: " + match + " New: " + roots[i]);
	...
	if (roots[i] != roots[currentRouteStart]) {
	...
	// if we have any hidden endpoints, might be able to combine
	// with neighbor(s)
