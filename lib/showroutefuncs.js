...
function hideShowToolbox() {
	...
	document.getElementById("listEntryToolbox").style.display = "";
	// if we have any selections, show them
	connections.forEach(c => {if (c.selected) createLESelectedOverlay(c);});
    }
    else {
	document.getElementById("listEntryToolbox").style.display = "none";
	// if we have any selections, remove their overlay
	connections.forEach(c => {if (c.selected) removeLESelectedOverlay(c);});
    }
}
...
function makeLEToolboxDraggable() {
    var pos1 = 0, pos2 = 0, pos3 = 0, pos4 = 0;
    var toolboxDiv = document.getElementById("listEntryToolbox");
    document.getElementById("lEToolboxHeader").onmousedown = dragMouseDown;
...
function showrouteMarkerClick(e, m) {
	...
	'</span>&nbsp;<button class="btn" data-clipboard-target="#srmctext">' +
	'Copy</button><br />' +
	'<b><a target="_blank" href="http://www.openstreetmap.org/?lat=' +
	coords.lat + '&lon=' + coords.lng + '">Coords.:</b> ' + coords.lat +
...
function createLESelectedOverlay(c) {
...
function removeLESelectedOverlay(c) {
...
function updateLEToolboxSelection() {
	...
	if (c.selected && c.selectedOverlay == null) {
	    createLESelectedOverlay(c);
	}
	if (!c.selected && c.selectedOverlay != null) {
	    removeLESelectedOverlay(c);
	}
    ...
    document.getElementById("lEToolboxLines").innerHTML = listEntries;
}
