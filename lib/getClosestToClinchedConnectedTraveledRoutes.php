...
// build the SQL query
$sql = "select r.firstRoot, r.route, r.banner, r.groupName, round(r.mileage,4) as mileage, round(cr.mileage,4) as traveled, (r.mileage - cr.mileage) as missing from connectedRoutes as r left join clinchedConnectedRoutes as cr on r.firstRoot=cr.route and traveler='".$params['traveler']."' join systems as s on s.systemName=r.systemName where cr.mileage > 0 and (r.mileage - cr.mileage) > 0";
