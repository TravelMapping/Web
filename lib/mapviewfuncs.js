...
function parseSegmentAndRouteData(data) {
    ...
    thead.innerHTML = '<tr><th rowspan="2">Route</th><th colspan="2">Traveled by ' + travelerInSpan() + ' ('+ distanceUnitsInSpan() + ')</th></tr><tr><th>Visible Segments</th><th>Route Overall</th></tr>';
	...
	if (routes[roots[i]].level == "devel") continue;
        ...
        // need to treat the coordinates as numbers, not strings
        w1lat[i] = parseFloat(w1lat[i]);
        w1lng[i] = parseFloat(w1lng[i]);
        w2lat[i] = parseFloat(w2lat[i]);
        w2lng[i] = parseFloat(w2lng[i]);
	...
	if (root == currentRoot) {
	    ...
	    // we try to do the merge unless these are special hidden points
	    // that could have travels that end at the point
	    if ((w1name[i][0] == '+') &&
		!w1name[i].startsWith('+DIV') &&
		!w1name[i].startsWith('+SKIP')) {
...
*/
    ...
    for (let tier = mapviewParams.maxTier; tier >= 1; tier--) {
	for (let i = 0; i < connections.length; i++) {
	    let c = connections[i];
	    if (c.TMminTier == tier) {
		c.addTo(map);
...
function showScrollableMapviewPopup() {
	...
	scrollableMapviewPopup = L.control.window(map, {
		...
		callback: function() {
		    ...
		    //    map.setView([setlat, setlon], setzoom);
		    // this is a "Cancel", in which case we will recreate
		    // the window next time
		    scrollableMapviewPopup = null;
		}
	    },
	    modal: true,
	    visible: true
	...
	let loninput = document.getElementById("lonvalinput");
	let zoominput = document.getElementById("zoomvalinput");
	if (placeinput != null) placeinput.value = "";
	let center = map.getCenter();
	if (latinput != null) latinput.value = center.lat;
	if (loninput != null) loninput.value = center.lng;
	if (zoominput != null) zoominput.value = map.getZoom();
