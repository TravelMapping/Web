...
*/
    ...
    // only one entry between min and max
    if (minTravelers + 2 == maxTravelers) {
	mapviewLegends.travelers +=
	    mapviewLegendEntry(segmentColorForAmountTraveled(minTravelers+1, maxTravelers), minTravelers);
    }
    // at least two entries between min and max, so we'll pick two of them
    if (maxTravelers - minTravelers > 2) {
	let value = Math.floor(minTravelers + (maxTravelers - minTravelers) / 3);
	mapviewLegends.travelers += 
	    mapviewLegendEntry(segmentColorForAmountTraveled(value, maxTravelers), value);
	value = Math.floor(minTravelers + 2 * (maxTravelers - minTravelers) / 3);
	mapviewLegends.travelers += 
	    mapviewLegendEntry(segmentColorForAmountTraveled(value, maxTravelers), value);
    }
    if (maxTravelers > minTravelers) {
	mapviewLegends.travelers +=
	    mapviewLegendEntry(segmentColorForAmountTraveled(maxTravelers, maxTravelers), maxTravelers);
    }
