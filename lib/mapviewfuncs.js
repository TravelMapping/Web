...
// Note: the scrollableMapvieDialog global variable is set in mapview.php,
...
function mapviewStartup(lat, lon, zoom) {
	...
	console.log("Load data for regions " + mapviewParams.regions.toString());
	// if we have regions, we could also have systems within
	if (mapviewParams.systems.length > 0) {
	    console.log("Restrict regions to systems " + mapviewParams.systems.toString());
	}
	else {
	...
	console.log("Load data for systems " + mapviewParams.systems.toString());
    }
...
function updateVisibleData() {
    ...
    // we will be in MAPVIEW mode at this point
...
// of the functionality of updateVisibleData
function parseVisibleData(data) {
...
// variables to be set by the callback function in updateMap, set with
// initial coordinates at Roger Bacon 321 at Siena College
...
function showScrollableMapviewPopup() {
	...
	scrollableMapviewPopup = L.control.window(map, {
		...
		action: function() {
		    ...
		    set_traveler_cookie();
		    set_units_cookie();
		    // restart call to updateMap, as we now return
		    // from this one (assuming we were called from there)
